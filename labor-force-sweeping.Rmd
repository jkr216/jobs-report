...
First, a `ggplot` recession object. 
...
Now let's run our models in one piped workflow, using `invoke_map` and `map` from `purrr`, and `tidy` from the `broom` package to clean up the results.
...
fcast_model_tbl %>%
  ...
  mutate(forecasted = map(modeled, forecast, h = 4)) %>% 
  mutate(tidied = map(forecasted, tidy)) %>% 
  unnest(tidied)

```

Try to visualize those results - there is no unified structure. Can't do it! 
